...
严格匹配部署的 2,893 条基线低于此前 2,909 条继承式门控诊断，这是预期差异：旧诊断融合
冻结 Visual OOF artifact 时漏掉了仅存在于发布包的 0.5 输出缩放，而测试推理通过 compact
package 实际应用了该缩放。Kaggle 0.97512 仍是事实观察，但 2,909 不是部署一致的 OOF 结果。
