...
[`quality_gate_ablation`](quality_gate_ablation/) 保留当前 Temporal/Visual 基础权重与
温度，只在双主输入有效样本上取消置信度逐样本加权。OOF 从 2,889 增至 2,893（纠正 6、
破坏 2；各折 +1/0/+1/+1/+1），但用户 bootstrap 区间跨 0，且两条破坏集中于用户 18。
它未通过预注册晋级门槛，因此不构建发布候选、不运行测试推理，也不继续扫描融合规则。
本轮配对比较与历史 strictV3 2,903、继承门控 T+V 2,909 不同，不更新上述结果或已发布的
0.97512 分数。实验目录已冻结预测、哈希和拒绝决策；当前 T+V 路径关闭，等待新证据。
冻结后经用户明确授权执行部署覆盖，gate-off CSV 以 ref 56117280 提交；公开榜 0.97512
持平不改变拒绝结论。
