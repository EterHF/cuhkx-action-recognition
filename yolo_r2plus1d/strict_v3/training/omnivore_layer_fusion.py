#!/usr/bin/env python3
"""Fine-tune multi-stage Depth/IR Omnivore fusion on one subject fold."""
...
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device) -> np.ndarray:
    model.eval()
    outputs = []
    for frames, _ in loader:
        frames = frames.to(device, non_blocking=True)
        depth = prepare_inputs(frames, "depth", False)
        infrared = prepare_inputs(frames, "ir", False)
        with torch.autocast(device.type, dtype=torch.bfloat16):
            outputs.append(model(depth, infrared).float().cpu().numpy())
...
def run(args: argparse.Namespace) -> None:
    ...
    model = OmnivoreLayerFusion(load_omnivore(args), load_omnivore(args)).to(device)
    fusion_parameters = (
        list(model.depth_projection.parameters())
        + list(model.ir_projection.parameters())
        + list(model.cross_attention.parameters())
        + list(model.stage_norm.parameters())
        + list(model.classifier.parameters())
        + list(model.depth_classifier.parameters())
        + list(model.ir_classifier.parameters())
    )
    optimizer = torch.optim.AdamW(
        (
            {"params": model.depth_trunk.parameters(), "lr": args.encoder_lr},
            {"params": model.ir_trunk.parameters(), "lr": args.encoder_lr},
            {"params": fusion_parameters, "lr": args.head_lr},
        ),
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for frames, target in train_loader:
            ...
            depth = prepare_inputs(frames, "depth", True)
            infrared = prepare_inputs(frames, "ir", True)
            optimizer.zero_grad(set_to_none=True)
            with torch.autocast(device.type, dtype=torch.bfloat16):
                logits, depth_logits, ir_logits = model(depth, infrared, return_aux=True)
                loss = criterion(logits, target)
                loss += args.auxiliary_weight * (
                    criterion(depth_logits, target) + criterion(ir_logits, target)
                )
            loss.backward()
    ...
    logits = evaluate(model, held_loader, device)
    ...
    metrics = {
        "protocol": "subject-held-omnivore-layer-fusion/v1",
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--auxiliary-weight", type=float, default=0.1)
