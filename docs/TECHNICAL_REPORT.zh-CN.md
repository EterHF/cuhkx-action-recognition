...
### 5.25 固定基础权重的置信度 quality-gate 消融，2026-09-09

§5.24 的覆盖结构只触发了一项复用 2,889/3,036 OOF、无拟合参数的探索性融合消融。
当前实现先计算各分支温度缩放后的最大 softmax 概率，经机会水平校正得到 quality 因子，
再乘冻结基础权重。候选只在双主输入有效样本上将 Temporal 与 Visual 的 quality 因子设为
相同常数；基础权重 0.67/0.22、温度 1.16518/1.52714、Visual 输出缩放 0.5、分支
logits 及全部缺失/部分缺失输出均保持不变。它与历史等权实验不同，没有修改 T/V 基础比例。

取消 quality 加权只改变 8 个预测：纠正 6、破坏 2，OOF 从 2,889 增至 2,893；各折
净变化为 +1/0/+1/+1/+1，用户宏平均从 0.950617 增至 0.952000，最差用户仍为
0.8125。53 个 Visual 独有 top-1 正确错误中修复 5 个，6 个 Temporal 独有正确中修复
1 个，两支 top-1 都错及双主输入缺失错误均未修复。代价是破坏 2/2,859 个原本正确的
双主输入有效样本。8 个变化全部发生在 T/V 分歧样本；2,655 个两支相同且唯一 top-1
样本及全部缺失/部分缺失样本均不变，数学一致性检查通过。

方向为正，但不足以晋级：双侧精确 McNemar p=0.2891，10,000 次按用户聚类 bootstrap
的 95% 区间为 [-0.000675, 0.003083]。两条破坏均来自用户 18，使该用户净 -2。由于
预注册 bootstrap 门槛失败，不构建发布路径候选，不运行匿名测试或提交，也不继续做
熵/margin 阈值、top-k 规则或其他融合扫描。当前只支持“直接取消 gate 有小幅探索性收益，
但没有充分的正向泛化证据”这一有限结论。

这些数字仅属于当前配对比较。历史 strictV3 2,903/3,036、继承门控 T+V
2,909/3,036 与当前 2,889→2,893/3,036 是三个不同的结果体系。因此 2,893 不是对任何
历史 OOF 的改善，也不更新已发布的 0.97512 public-score 声明。保留基线；完整预测、输入
哈希、配对变化、用户聚类区间和拒绝决策均归档于
`results/experiments/quality_gate_ablation/`。当前 T+V 融合实验链关闭，直到新证据支持一个
新的、可独立检验的机制。

冻结完成后，用户明确授权覆盖“不提交”这一外部执行决定。与部署匹配的 gate-off 预测相对
继承门控 T+V 仅改变 405 行中的 1 行，未使用匿名标签；Kaggle ref 56117280 得分
0.97512。公开榜持平不使失败的 OOF 门槛事后通过、不晋升候选、不更新发布基线，也不授权
继续调参。
