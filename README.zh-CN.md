...
后续 NTU 规模实验使用固定 seed 2026，分别从 Kinetics+NTU120、NTU60/NTU120
等权源混合和 NTU60 control 重新训练目标模型。可部署的 NTU120 成员通过 INT4
train-only 门禁：strictV3 90/10 融合保持 2,903/3,036 OOF、worst-user 下限
0.8125，并改变 10 个 OOF 预测。两次 package 重放完全一致，改变匿名索引 133；
Kaggle 提交 `56014518` 得到 **0.97512**，仍是持平而非提升。

源混合将 external INT4 OOF 从 1,859 提升到 1,910 行，但最终 CSV 与已提交的
PKU 诊断逐字节相同，因此没有重复提交。Target-weight soup 未通过 external OOF
门禁；重新训练的 NTU60 control 也未达到冻结的 INT4 external 阈值（1,875 <
1,900），二者均未访问匿名数据。当天仍有 3 次配额，但没有其他候选同时通过证据和
新颖性门禁，因此没有为消耗次数而盲目提交。
