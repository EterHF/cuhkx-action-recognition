...
from yolo_r2plus1d.strict_v3.evaluation.quality_gate_submission import (
    gate_off_predictions,
)
...
def test_gate_off_keeps_control_on_invalid_rows() -> None:
    visual = np.asarray([[2.0, 0.0], [2.0, 0.0]], dtype=np.float32)
    temporal = np.asarray([[0.0, 4.0], [0.0, 4.0]], dtype=np.float32)
    release = {
        "weights": {"fusion": 0.0, "visual": 1.0, "temporal": 3.0},
        "temperatures": {"fusion": 1.0, "visual": 2.0, "temporal": 4.0},
    }
    control, candidate = gate_off_predictions(
        visual, temporal, np.asarray([True, False]), release
    )
